...
5. Título é buscado automaticamente (`fetch` + parse do `<title>`)
6. Usuário edita título, adiciona comentário (opcional) e tags
7. Clica **Salvar no Kura** → link gravado no IndexedDB → toast de confirmação → popup fecha
